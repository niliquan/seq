$(document).ready(
      function(){$('#session').login();
          ...
          if(p.hasClass('current')){p.removeClass('current');
          ...
          if($('#ask').length>0){$('#top-nav a.btn-regular').addClass('btn-disabled').click(function(){return false;});}
          $('.tip-pop').each(function(){var t=$(this),str=t.data('tip');
          if(!!!str){return;}
          t.removeAttr('title');
          var parts=str.split(':');
          $(this).tipsy({'html':true,'gravity':parts.shift(),'fallback':parts.join(':')});});
          if(login){$('#main').highlightTag({url:'http://x.segmentfault.com/tag/following',selector:'#content article .meta-tags li a, #user-question article .meta-tags li a',className:'q-highlight'});var accountInfo=$('#my-account .info');
          if(accountInfo.length>0){accountInfo.animate({backgroundColor:'#9a4444',color:'#fff'},'slow',function(){$(this).animate({backgroundColor:'#d9edf7',color:'#326aa8'},'slow');});}}
          else if(0==$('.auth-login,.session-form,.session-finished').length){
         	if(1!=$.cookie('sfln_viewed')){$.cookie('sfln_viewed',1,{path:'/'});
                                   	$.cookie('sfln_available',1,{path:'/'});
                                                                                                	}
          else if(1==$.cookie('sfln_available')){
      	                     $('.i-cancel',$('.login-notify').css('bottom',-60).removeClass('hidden').animate({'bottom':0})).click(function(){$.cookie('sfln_available',0,{path:'/'});
                            $(this).parents('.login-notify').animate({'bottom':-60},function(){$(this).remove();});
                            return false;});}}
